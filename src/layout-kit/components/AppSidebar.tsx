              ...
              <SidebarMenuSubItem key={`${subItem.title}-${subItem.url}`}>
                <SidebarMenuSubButton asChild isActive={isActive(subItem.url)} className="h-8 rounded-md">
                  <Link href={subItem.url}>
                    ...
                    {subItem.badge && <Badge tone="neutral" className="ml-auto h-5 px-1.5 text-[10px] font-semibold">{subItem.badge}</Badge>}
                  </Link>
                </SidebarMenuSubButton>
              </SidebarMenuSubItem>
            )
      ...
      if (!hasNested) {
        ...
        return (
          ...
          >
              ...
              {subItem.badge && (
                <Badge tone="neutral" className="h-5 px-1.5 text-[10px] font-semibold">
                  {subItem.badge}
                </Badge>
              )}
                          ...
                          <CollapsibleTrigger asChild>
                            <SidebarMenuButton tooltip={item.title} isActive={isSubActive(item.items)} className={cn("h-10 rounded-lg transition-colors", collapsedIconButtonClass)}>
                              ...
                              {item.badge && !isCollapsed && (
                                <Badge tone="neutral" className="ml-auto h-5 px-1.5 text-[10px] font-semibold">{item.badge}</Badge>
                              )}
